...
export default class I18n {
    ...
    async load() {
        this.translations = await invoke('plugin:i18n|load_translations');
        this.locale = await invoke('plugin:i18n|get_locale');
        // Bind existing elements
        this.autoBind();
        const unlisten = await listen('i18n:locale_changed', (event) => {
  ...
  */
    ...
    static async setLocale(locale) {
        await invoke('plugin:i18n|set_locale', {
            locale: locale
        });
    }
    /**
    ...
    static async getLocale() {
        const locale = await invoke('plugin:i18n|get_locale');
        return locale;
    }
    /**
     * **getAvailableLocale**
     ...
     * ```
     */
    static async getAvailableLocales() {
        const locale = await invoke('plugin:i18n|get_available_locales');
        return locale;
    }
    // Clean up when done
    destroy() {
